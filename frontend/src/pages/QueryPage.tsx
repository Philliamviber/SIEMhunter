...
import { useQueryHistory, useAddQueryHistory } from '../hooks/useApi';
...
export function QueryPage() {
  ...
  const addHistory = useAddQueryHistory();

  async function runQuery(overrideSql?: string) {
    const queryStr = overrideSql ?? sql;
    if (!queryStr.trim()) return;
    if (overrideSql) setSql(overrideSql);
    setLoading(true);
    setError(null);
    setResult(null);
    try {
      const res = await api.query({ sql: queryStr });
      setResult(res);
      addHistory.mutate(queryStr);
    } catch (e) {
  ...
  const viewFilters: Record<string, unknown> = sql.trim() ? { sql } : {};

  function loadSavedSql(filters: Record<string, unknown>) {
    if (typeof filters.sql === 'string') {
      setSql(filters.sql);
      setError(null);
      setResult(null);
    }
  ...
  return (
    <div className="p-6 flex flex-col gap-5">
      ...
      <div className="flex gap-5">
        {/* Left sidebar: saved views + history */}
        <div className="w-56 flex-shrink-0 flex flex-col gap-3">
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-lg p-3">
            <span className="text-xs font-medium text-gray-300 block mb-2">Recent History</span>
            {historyEntries.length === 0 ? (
              <div className="text-xs text-gray-600">No history yet</div>
            ) : (
              <ul className="space-y-1" role="list" aria-label="Query history">
                {historyEntries.map((entry, idx) => (
                  <li key={`${entry.run_at}-${idx}`} className="group">
                    <div className="flex items-start justify-between gap-1">
                      <button
                        onClick={() => runQuery(entry.sql)}
                        className="flex-1 text-left text-xs text-gray-400 hover:text-white transition-colors truncate py-0.5 font-mono"
                        title={entry.sql}
                        aria-label={`Re-run: ${entry.sql.slice(0, 60)}`}
                      >
                        {entry.sql.length > 50 ? `${entry.sql.slice(0, 50)}…` : entry.sql}
                      </button>
                    </div>
                    <div className="text-gray-700 text-xs leading-none mb-1">
