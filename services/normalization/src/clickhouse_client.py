"""
ClickHouse client shared by normalization, detection, and api services.
...
def insert_events(client: Client, rows: list[dict]) -> None:
    """Parameterized bulk INSERT into security_events."""
    if not rows:
        return
    client.insert(
        "security_events",
    ...
    log.debug("events_inserted", count=len(rows))
